...
```
CREATE TABLE IF NOT EXISTS _DESEMBARGO.DESEMBARGOS_IDAF_ES(
	...
	responsavel_desembargo TEXT NOT NULL REFERENCES _DESEMBARGO.USERS_INFO(USERNAME),
	descricao TEXT,
	status TEXT CHECK (status IN ('APROVADO','EM ANÁLISE','REVISÃO PENDENTE')),
	aprovado_por TEXT REFERENCES _DESEMBARGO.USERS_INFO(USERNAME),
...
```
CREATE INDEX NOME_INDEX ON SCHEMA.DESEMBARGOS_TESTE(id);
CREATE INDEX NOME_INDEX ON SCHEMA.EMBARGOS_TESTE(n_iuf_emb);
CREATE INDEX NOME_INDEX ON SCHEMA.EMBARGOS_LEGACY_TESTE(n_iuf_emb);

CREATE INDEX NOME_INDEX ON SCHEMA.EMBARGOS_TESTE(processo);
CREATE INDEX NOME_INDEX ON SCHEMA.EMBARGOS_LEGACY_TESTE(numero_sep);
 	
CREATE INDEX NOME_INDEX ON SCHEMA.USERS_TEST(username);
```
