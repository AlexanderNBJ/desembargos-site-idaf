...
(async () => {
  try {
    await runMigration();
    const id1 = await upsertUser('gerente@example.com', 'gerente123', 'GERENTE', 'Gerente', 'Gerente');
    const id2 = await upsertUser('comum1@example.com', 'comum123', 'COMUM', 'Comum1', 'Secretário');
    const id3 = await upsertUser('comum2@example.com', 'comum123', 'COMUM', 'Comum2', 'Secretário');
